...
# 读取控制数据
control_data_path = "C:\CARLA_0.9.14\WindowsNoEditor\PythonAPI\examples\SSY226_Project-saeed\save_car_control.txt"
control_data = np.loadtxt(control_data_path)  # 从文件读取控制数据

control_data_path_truck = "C:\CARLA_0.9.14\WindowsNoEditor\PythonAPI\examples\SSY226_Project-saeed\save_truck_control.txt"
control_data_truck = np.loadtxt(control_data_path_truck)  # 从文件读取控制数据

velocity1 = carla.Vector3D(56, 0, 0)
...
for i in range(len(control_data[25:])):
    ...
    if control_data[i + 25][0] <0:
        ...
        car_brake_input =  -np.sin(control_data[i + 25][0]) # 使用非线性函数并保证刹车输入在[0, 1]之间
        car_throttle = 0  # 油门设为0
    else:
        # 否则，继续使用估计的油门值
        car_throttle = np.sin(control_data[i + 25][0])  # 使用非线性函数并保证油门输入在[0, 1]之间
        car_brake_input = 0
    
    car_steer_input = np.sin(control_data[i + 25][1])   # 假设给Car设置的方向盘值
    if control_data_truck[i + 25][0] <0:
    # 如果估计的油门小于0，意味着需要减速或刹车
        truck_brake_input =  -0*np.sin(control_data_truck[i + 25][0]) # 使用非线性函数并保证刹车输入在[0, 1]之间
