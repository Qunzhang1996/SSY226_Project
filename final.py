...
car_bp = bp_lib.find('vehicle.mini.cooper_s_2021')
...
#     wheel.tire_friction = 10  # 可以根据需要调整这个值
...
def normalization(x, x_min=-35, x_max=35):
    #x here is rads, have to convert it to degrees
    x=x*180/np.pi
        ...
        for _ in range(steps_between_replanning):
            for v in [v1, v2]:
                ...
                if v==v1:#control the truck
                    ...
                    estimated_throttle = u_optimal[0][0]
                    ...
                    truck_control = carla.VehicleControl(throttle=throttle_input, steer=steer_input, brake=brake_input)
                    truck.apply_control(truck_control)  
                    # truck.apply_control(carla.VehicleControl(throttle=throttle_input, steer=steer_input, brake=brake_input)) 
                    ...
                    if steer_input < 0:
                        ...
                        throttle_input = np.sin(estimated_throttle)
                        brake_input = 0
                    car_control = carla.VehicleControl(throttle=throttle_input, steer=steer_input, brake=brake_input)
                    car.apply_control(car_control)  
                    # car.apply_control(carla.VehicleControl(throttle=throttle_input, steer=steer_input, brake=brake_input)) 
