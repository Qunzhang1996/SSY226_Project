...
def main():
    ...
    # CC Truck
    v1 = Truck_CC([18, 24, 143.318146+lane_width, 0],dt=dt)
    ...
    v2 = Car_km([15, 124, 143.318146, 0],dt=dt)
    ...
    plt.plot(v1_history_state[ST.S][-1], v1_history_state[ST.N][-1], 'o', color=line_v1.get_color(), label='Truck Actual Trajectory')
    line_v2, = plt.plot(v2_history_state[ST.S], v2_history_state[ST.N])
    plt.plot(x_data, y_data, 'o', color=line_v2.get_color(), label='Car Actual Trajectory')

    for vhp in v1_history_planned:
        plt.plot(vhp[ST.S, :], vhp[ST.N, :], ':',
                color=line_v1.get_color(), linewidth=0.7)

    plt.plot([], [], ':', color=line_v1.get_color(), linewidth=0.7, label='Truck Planned Trajectory')

    for vhp in v2_history_planned:
        plt.plot(vhp[ST.S, :], vhp[ST.N, :], ':',
                color=line_v2.get_color(), linewidth=0.7)
        
    plt.plot([], [], ':', color=line_v2.get_color(), linewidth=0.7,label='Car Planned Trajectory')

    plt.subplots_adjust(hspace=0.5) 
    plt.legend( loc='upper right')
    plt.subplot(2, 1, 2)
    plt.title("Vehicle Speeds vs. Longitudinal Distance")
    line_v1, = plt.plot(v1_history_state[ST.S], v1_history_state[ST.V], label='Truck Actual Trajectory')
    line_v2, = plt.plot(v2_history_state[ST.S], v2_history_state[ST.V], label='Car Actual Trajectory')
    for vhp in v1_history_planned:
